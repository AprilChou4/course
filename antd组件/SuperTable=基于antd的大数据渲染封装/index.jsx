...
class SuperTable extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (userAgent.indexOf('Chrome') > -1 && userAgent.indexOf('Safari') > -1) {
      this.isNoDataClassName =
        this.isNoDataClassName.indexOf('isChromeClass') > -1
          ? this.isNoDataClassName
          : 'isChromeClass';
    }
    this.bindResize();
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    const {
      columns: oldColumns,
      refreshColumns: oldRefreshColumns,
      refreshTable: oldRefreshTable,
    } = this.props;
    ...
    if (oldColumns !== nextProps.columns || this.compareRowSelection(nextProps, this.props)) {
      ...
      if (nextProps.refreshTable && oldRefreshTable !== nextProps.refreshTable) {
        this.create();
      }
      ...
      this.setState({ columns }, () => {
        if (
          nextProps.dragCell &&
          (!this.computed ||
            this.compareColumnsDiff(nextProps.columns, oldColumns) ||
            nextProps.refreshColumns !== oldRefreshColumns)
        ) {
  ...
  setYHeight() {
    const { tableWrapper } = this;
    const { isSummaryTable } = this.props;
    // 合计行不计算高度，默认高度58
    if (isSummaryTable) return;
    const isSummary = tableWrapper.getAttribute('class').indexOf('summary-table') !== -1;
    ...
    let y = 0;
    let wraperMaxHeight = 0;
    if (tableWrapper.parentNode) {
      y = tableWrapper.parentNode.offsetHeight;
      wraperMaxHeight = tableWrapper.parentNode.style.maxHeight;
    }
    // 表格高度根据父元素高度自适应增加
    if (wraperMaxHeight) {
    ...
    if (rowSelection) {
      ...
      return {
        ...
        title: () => {
          ...
          const { filterVisibleDataSource } = this.props;
          dataSource = this.filterDisabledDataSource(dataSource, getCheckboxProps);
          // 复选框合并后全选按钮问题修复
          const visibleDataSource =
            rowSpan && filterVisibleDataSource
              ? filterVisibleDataSource(dataSource).filter((ele) => ele.rowSpan !== 0)
              : dataSource;
          const checked =
  ...
  computedColumns(cols) {
    const { minWidthCount, overLength } = this.props;
        ...
        if (container) {
          ...
          if (result !== 0) {
            ...
            dragColumns.forEach((ele, key) => {
              const newWidth = overLength ? ele.width : ele.width + averageWidth;
  ...
  getColumns({ dragCell, columns, rowSelection }) {
    ...
    if (firstCol && firstCol.type === 'pre') {
      ...
      newColumns = selectionCol
        ? [{ ...firstCol, dragCell: false, fixed: 'left' }, selectionCol, ...rest]
        : [{ ...firstCol, dragCell: false, fixed: 'left' }, ...rest];
    } else {
      newColumns = selectionCol ? [selectionCol, ...columns] : [...columns];
    }
  ...
  setTranslateY(dom) {
    ...
    if (dom) {
      ...
      prefix.forEach((pre) => {
        cssText += `${pre}transform: translateY(${translateY}px);`;
      });
  ...
  render() {
    ...
    const { dataSource, columns, y, scrolling } = this.state;
...
SuperTable.defaultProps = {
  ...
  refreshColumns: undefined, // 刷新 columns 标记
  setRecalculate: undefined, // 重新计算高度等标记，传如时间戳等
  noLazyLoading: false, // 是否禁止懒加载
  overLength: false, // 拖拽表格宽度总宽度超过屏幕宽度是否自适应 false==>自适应
  isSummaryTable: false, // 是否是底部合计表格，用于解决合计列高度变化问题
};
...
SuperTable.propTypes = {
  ...
  refreshColumns: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  setRecalculate: PropTypes.oneOfType([PropTypes.string, PropTypes.number, PropTypes.object]),
  noLazyLoading: PropTypes.bool,
  overLength: PropTypes.bool,
  isSummaryTable: PropTypes.bool,
};
