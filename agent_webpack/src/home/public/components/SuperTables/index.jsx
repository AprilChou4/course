...
class SuperTable extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    // 防止页面加载出来时高度计算错误问题
    if (dataSource.length && !this.setY) {
      this.setY = true;
      this.setYHeight();
    }
    // 当dataSource有改变
  ...
  setDataSource(state, cb) {
    const { dataSource, bottomScrollBar } = this.props;
    const visibleDataSource = this.getDataSource(dataSource);
    ...
    if (rowSelection) {
      ...
      return {
        ...
        title: () => {
          ...
          return (
            <Checkbox
              ...
              onChange={this.selectAllRow.bind(this, dataSource)}
            >
              {columnTitle}
            </Checkbox>
          );
        ...
        render: (text, record) => {
          ...
          if (record.rowSpan && rowSelection.rowSpan) {
            data.props = {
  ...
  getColumns({ dragCell, columns, rowSelection }) {
    ...
    if (selectionCol) {
      ...
      if (firstCol && firstCol.type === 'pre') {
        newColumns = [{ ...firstCol, dragCell: false }, selectionCol, ...rest];
      } else {
  ...
  scrollHandle() {
    ...
    const { rowHeight, onScroll } = this.props;
  ...
  render() {
    ...
    const props = {
      ...rest,
      ref: this.tableRef,
      className: classnames('ant-supertables', className, dragCell ? 'dragtable' : ''),
