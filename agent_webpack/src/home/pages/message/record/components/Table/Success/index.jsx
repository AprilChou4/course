...
@connect(({ current, total, pageSize, dataSource, loadings, isDetailVisible }) => ({
  current,
  total,
  pageSize,
  dataSource,
...
class SuccessTable extends PureComponent {
  constructor(props) {
    super(props);
    this.columns = [
      {
  ...
  render() {
    const { total, pageSize, current, loadings, isDetailVisible, ...rest } = this.props;
    return (
      <Fragment>
        <SuperTable
          ...
          loading={loadings.$getSuccessList}
...
export default connect(({ query }) => ({ query }))(SuccessTable);
